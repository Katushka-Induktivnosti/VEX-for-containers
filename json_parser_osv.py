...
def create_df():
    df = pd.DataFrame(columns=['VulnerabilityID','PkgName','Status','Severity','Container','tool'])
    files = [f for f in os.listdir('jsons/OSV/syft_sbom_vex/')]
        ...
        try:
            ...
            for package in data['results'][0]['packages']:
                for vuln in package['vulnerabilities']:
                    ...
                    df.loc[len(df)] = [id_name, pkgname, status, severity, container_name,'trivy']
        except:
            continue
        f.close()
    return df
